...
export type CommandListMutable = {
  ...
  runAs?: string // Login for a user on whose behalf the command is executed. Can be null.
...
export type ParsedCommand = EntityBase<"ParsedCommand"> & {
  description?: string // HTML description of the command that describes what will be done to issue as a result of this command. Read-only. Can be null.
  error?: boolean // Indicates whether the command contains errors. Read-only. Can be null.
  delete?: boolean // Indicates whether the command deletes an issue. Read-only. Can be null.
}
