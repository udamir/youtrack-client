...
export type SystemSettings = EntityBase<"SystemSettings"> & {
  baseUrl: string | null // Base URL of the YouTrack installation.
  ...
  administratorEmail: string | null // Email of server administrator.
...
export type EmailSettings = EntityBase<"EmailSettings"> & {
  ...
  host: string | null // Mail server that is used for notification.
  ...
  login: string | null // Mail server login.
  sslKey: StorageEntry | null // SSL key to be used for accessing the mail server.
  from: string // From address.
  replyTo: string | null // Reply-to address.
}

export type StorageEntry = EntityBase<"StorageEntry"> & {
  name: string | null // Name of the SSL key.
}
...
export type Telemetry = EntityBase<"Telemetry"> & {
  installationFolder: string | null // Installation folder of YouTrack.
  databaseLocation: string | null // Location of the YouTrack database.
  logsLocation: string | null // Location of the logs.
  availableProcessors: number | null // Number of available processors.
  availableMemory: string | null // Available memory.
  allocatedMemory: string | null // Allocated memory.
  usedMemory: string | null // Used memory.
...
export type DatabaseBackupSettings = EntityBase<"DatabaseBackupSettings"> & {
  location: string | null // The location of the backups storage folder.
  filesToKeep: number // The number of backup files that should be stored.
  cronExpression: string | null // The cron expression that defines the schedule of the automatic backups.
...
export type BackupStatus = EntityBase<"BackupStatus"> & {
  ...
  backupError: BackupError | null // The error that appeared during the previous backup process. Returns null, if there was no error.
}

export type BackupError = EntityBase<"BackupError"> & {
  date: number | null // The timestamp in milliseconds indicating the moment when the error appeared. Stored as a unix timestamp at UTC.
  errorMessage: string | null // Description of the error.
}
