 ...
 */
export type ParseSchema<T extends string> = 
  ParseSubSchema<T> extends [infer Schema, infer Rest]
    ? Rest extends ""
      ? Schema
      : never
    : never
 ...
 */
type FindName<T, Name extends string = ""> =
  T extends `${Name}${infer Char}${infer Rest}`
    ? Char extends "," | "(" | ")"
      ? [Name, Char, Rest]
 ...
 */
type ParseSubSchema<T, Items extends FieldsSchema = []> = 
  FindName<T> extends [infer Name, infer Char, infer Rest]
    ? Name extends string
      ? Char extends ","
        ...
        : Char extends "("
          ? ParseSubSchema<Rest> extends [infer SubSchema, infer Rest2]
            ...
            : never
          : Char extends ")"
            ? Name extends "" ? [Items, Rest] : [[...Items, Name], Rest]
            : never
      : never
