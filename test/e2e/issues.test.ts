...
describe("Basic issue cases", () => {
  it("should create and delete test issue in test project", async () => {
    if (!enableUpdates) {
      console.log(
        "Skipping issue creation test - ENABLE_UPDATES is set to false",
      );
      return;
    }
    ...
    if (!TEST_PROJECT_ID) {
      console.error(
        "TEST_PROJECT_ID is not defined in .env file. Please add a valid project ID.",
      );
      return;
    }
  ...
  it("should get issue by ID", async () => {
    ...
    try {
      ...
      if (issues.length > 0) {
        ...
        const issue = await yt.Issues.createIssue(
          {
            ...
            description:
              "This is a test issue created to test the getIssueById API",
          },
          { fields: "id" },
        );
        ...
        console.log(
          "Skipping getIssueById test - no issues found and ENABLE_UPDATES is false",
        );
        return;
      }
  ...
  it("should add work item", async () => {
    ...
    try {
      ...
      const projectTimeTrackingSettings =
        await yt.Admin.Projects.getProjectTimeTrackingSettings(
          TEST_PROJECT_ID,
          {
            fields: "enabled",
          },
      ...
      expect(projectTimeTrackingSettings.enabled).toBe(true);
      ...
      const fetchedIssueWorkItems =
        await yt.IssueTimeTracking.getIssueWorkItems(testIssueId, {
          fields: "text,duration(presentation,minutes)",
        });
      const createdWorkItem = fetchedIssueWorkItems.find(
        (wi) => wi.text === workItemText,
      );
